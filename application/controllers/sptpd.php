...
class Sptpd extends MY_Controller
{
    ...
    public function add_by_wp()
    {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $nik          = $this->input->post('txt_id_nik_sptpd');
                $wajibpajak   = $this->input->post('wajibpajak') . " " . $this->input->post('wajibpajaktext') . " " . $this->input->post('wajibpajaklainnya');
                $wajibpajak   = str_replace("'", "`", $wajibpajak);
                $nomor        = $this->global->nomor('sspd');
                ...
                if ($bea_bayar <= $jml_setor or $dasar_jml_setoran == 'SKBKB' or $dasar_jml_setoran == "SKBKBT") {
                    ...
                    if (@$info) {
                        ...
                        $body1 = 'Anda Telah dipilih notaris oleh Bpk/Ibu/Sdr ' . $nama_nik . ', silahkan cek di menu SSPD untuk verifikasi berkas';
                        ...
                        $insert_tbl_formulir_penelitian = $this->db->insert('tbl_formulir_penelitian', $paramFormulir);
                        file_put_contents(FCPATH . "/assets/log/insert_formulir_cek.log", $this->db->last_query() . PHP_EOL, FILE_APPEND);

                        if (!$insert_tbl_formulir_penelitian) {
                            file_put_contents(FCPATH . "/assets/log/insert_formulir.log", "ERROR : " . $this->db->error() . "_SQL : " . $this->db->last_query() . PHP_EOL, FILE_APPEND);
                        }
    ...
    public function uploadFile($file = '', $name_fix = '', $no_dokumen = '', $allowed_types = 'png|jpg|jpeg|pdf')
    {
        ...
        if (!file_exists(str_replace('index.php', '', $_SERVER['SCRIPT_FILENAME']) . $new_folder . '/index.php')) {
        ...
        if (!$this->upload->do_upload($file)) {
            ...
            file_put_contents(FCPATH . "assets/log/gagal_upload.log", json_encode(array(
                "file"          => $file,
                "name_fix"      => $name_fix,
            ...
            file_put_contents(FCPATH . "assets/log/sukses_upload.log", json_encode(array(
                "file"          => $file,
                "name_fix"      => $name_fix,
                "no_dokumen"    => $no_dokumen,
                "msg"           => $data
    ...
    public function printform($id)
    {
        ...
        if ($kec == '' or $kel == '') {
            die("Error : Kode Kecamatan atau Kode Kelurahan Bernilai Kosong, Data SPTPD Kemungkinan Tidak Valid, Informasikan Ke Admin Bapenda Untuk Penghapusan SPTPD Ini, Terimakasih!");
        }

        $this->db->select("b.alamat, b.harga");
        $this->db->from("tbl_sptpd a");
        $this->db->join("harga_refrensi b", "b.kd_kec = a.kd_kecamatan and b.kd_kel = a.kd_kelurahan");
        $this->db->where("a.kd_propinsi", "35");
        $this->db->where("a.kd_kabupaten", "73");
        $this->db->where("b.kd_kec", $kec);
        $this->db->where("b.kd_kel", $kel);
        $this->db->where("a.id_sptpd", $id);
        $data['hr'] = $this->db->get();
        ...
        $data['sptpd']->qr_image = base_url('assets/images/qris-default.jpg');
        $data["lhp_verlap_file_exist"] = (file_exists(FCPATH . "/assets/files/penelitian/" . str_replace(".", "", $data['sptpd']->no_dokumen) . "/lhp_verlap_" . $id . ".jpg")) ? "lhp_verlap_" . $id . ".jpg" : ((file_exists(FCPATH . "/assets/files/penelitian/" . str_replace(".", "", $data['sptpd']->no_dokumen) . "/lhp_verlap_" . $id . ".pdf")) ? "lhp_verlap_" . $id . ".pdf" : "");
        $data["lampiran_lain_file_exist"] = (file_exists(FCPATH . "/assets/files/penelitian/" . str_replace(".", "", $data['sptpd']->no_dokumen) . "/lampiran_lain_" . $id . ".jpg")) ? "lampiran_lain_" . $id . ".jpg" : ((file_exists(FCPATH . "/assets/files/penelitian/" . str_replace(".", "", $data['sptpd']->no_dokumen) . "/lampiran_lain_" . $id . ".pdf")) ? "lampiran_lain_" . $id . ".pdf" : "");
        if ($data['sptpd']->qr_code != "") {
            $this->load->library("qris");
            $data['sptpd']->qr_image = $this->qris->generateQRIS($data['sptpd']->qr_code, "url");
        }
    ...
    {
        if ($this->input->post('txt_id_nik_sptpd')) {
            $id_nik = $this->input->post('txt_id_nik_sptpd');
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($bea_bayar <= $jml_setor or $dasar_jml_setoran == 'SKBKB' or $dasar_jml_setoran == "SKBKBT") {
                    ...
                    if (@$info) {
                        ...
                        //-------FAIZAL--------//
                        //INPUT LAMPIRAN SPOP/LSPOP//
                        if ($_FILES['txt_picture_lampiran_spoplspop']['name'] != '') {

                            $file = $this->uploadFile('txt_picture_lampiran_spoplspop', $no_dokumen_file . '_lampiran_spoplspop', $no_dokumen_file);
	...
	*/
    ...
    public function anu()
    {
        // print_r($this->mod_sptpd->get_nik_user_kabid($this->session->userdata('s_id_user'))->jabatan);
        $timestamp  = strtotime(date("Y-m-d H:i:s"));
        print_r(base_url("assets/files/sspd_bphtb/$id_sptpd/signed_$timestamp.pdf"));
    }

    public function proses_validasi_akhir($id_sptpd = "", $is_bulking = false)
    {
        ...
        $jabatan        = $this->mod_sptpd->get_nik_user_kabid($this->session->userdata('s_id_user'))->jabatan;
        $nik_validator  = $this->mod_sptpd->get_nik_user_kabid($this->session->userdata('s_id_user'))->nik;
        if (empty($nik_validator)) {
            $response = array(
                "msg" => "NIK validator tidak diketahui."
            );
        } elseif ($jabatan != 2) {
            $response = array(
                "msg" => "User tidak berhak melakukan validasi dan melakukan proses tanda tangan elektronik."
            );
        } else {
            if ($id_sptpd != "") {
                if ($data) {
                    $nik_tte        = $data["nik_tte"];
                    $passphrase_tte = $data["passphrase_tte"];

                    if ($this->mode == "development") {
                        $response = array(
                            "status"    => true,
                            "msg"       => "Proses TTE berhasil.",
                        );
                    } else {
                        #0 buat qrcode untuk lokasi file nantinya
                        $timestamp          = strtotime(date("Y-m-d H:i:s"));
                        $this->load->library("qr");
                        $qr_file_name = $this->qr->generateQR(base_url("assets/files/sspd_bphtb/$id_sptpd/signed_$timestamp.pdf"), "file_name");

                        #1 simpan pdf dengan spesimen tte ke folder yang ditentukan berdasarkan id_sptpd
                        $createPDFUnsigned  = $this->sspdbphtbpdfdownload($id_sptpd, FCPATH . "assets/files/sspd_bphtb/$id_sptpd", "unsigned_$timestamp.pdf", $timestamp, $qr_file_name, $nik_validator);

                        #2 lakukan proses tte
                        $this->load->library("Esign", "esign");
                        $data_proses = array(
                            "full_path"         => $createPDFUnsigned["full_path"],
                            "timestamp"         => $createPDFUnsigned["timestamp"],
                            "nik_tte"           => $nik_tte,
                            "passphrase_tte"    => $passphrase_tte,
                        );

                        $proses_tte = $this->esign->proses_tte($data_proses);

                        #3 cek apakah proses tte berhasil atau tidak
                        if ($proses_tte["status_code"] == 200) {
                            #4 simpan data blob ke lokasi folder yang ditentukan berdasarkan id_sptpd
                            $file_name_signed = "signed_$timestamp.pdf";
                            file_put_contents(FCPATH . "assets/files/sspd_bphtb/$id_sptpd/" . $file_name_signed, $proses_tte["data_blob_pdf"]);

                            #5 simpan nama file kedalam database berdasarkan id_sptpdnya
                            $data_to_update = array(
                                "tgl_validasi"      => date("Y-m-d H:i:s"),
                                "nik_user_validasi" => $nik_validator,
                                "file_validasi"     => $file_name_signed,
                            );
                            $this->mod_sptpd->update_file_validasi($id_sptpd, $data_to_update);

                            #5.1 update status dari lunas menjadi validasi
                            $data = $this->pembayaran->validasi_akhir($id_sptpd);

                            #6 hapus file yang belum di tanda tangani
                            unlink(FCPATH . "assets/files/sspd_bphtb/$id_sptpd/unsigned_$timestamp.pdf");

                            #6.1 hapus qrcode yang ngelink menuju file sspd
                            unlink(FCPATH . "assets/images/qrcode/$qr_file_name");
                            ...
                            #7 hapus pdf yang belum ttd dan juga foldernya
                            $this->deleteFolder(FCPATH . "assets/files/sspd_bphtb/$id_sptpd");

                            #8 hapus qrcode menuju file sspd
                            unlink(FCPATH . "assets/images/qrcode/$qr_file_name");

                            $response = array(
                                "msg"       => "Proses TTE Gagal Karena " . json_decode($proses_tte["error_message"])->error . ".",
                            );
                        }
                    }
                } else {
                    $response = array(
                        "msg" => "Server tidak menerima data nik dan passphrase."
                    );
                }
            } else {
                $response = array(
                    "msg" => "Server tidak menerima data ID SPTPD."
                );
            }
        }
    ...
    private function sspdbphtbpdfdownload($id, $fileLocation, $fileName, $timestamp, $qr_file_name, $nik_validator)
    {
        $data['id']       = $id;
        $data['rekening'] = $this->mod_rekening->get_rekening(1);
        $data['sptpd']    = $this->mod_sptpd->get_sptpd($id);
        ...
        foreach ($tujuan as $key => $value) {
            ...
            $imageTTE = base_url() . 'assets/images/spesiment/' . $nik_validator . '.jpg';
            ...
            $qr_file = base_url() . 'assets/images/qrcode/' . $qr_file_name;
        ...
        $this->createFolderIfNotExists($fileLocation);
        $status = $pdf->Output("F", $fileLocation . "/" . $fileName);

        return array(
            "full_path" => $fileLocation . "/" . $fileName,
            "file_name" => $fileName,
    ...
    {
        // Pastikan folder path diakhiri dengan slash
        $folderPath = rtrim($folderPath, '/') . '/';

        // Cek apakah folder path tersebut adalah folder
        if (is_dir($folderPath)) {
            // Buka folder
            $folderHandle = opendir($folderPath);

            // Loop melalui semua file dan subfolder di dalam folder
            while (($file = readdir($folderHandle)) !== false) {
                // Lewati . dan ..
                if ($file !== '.' && $file !== '..') {
                    $filePath = $folderPath . $file;

                    // Jika file path adalah folder, lakukan rekursi
                    if (is_dir($filePath)) {
                        deleteFolder($filePath);
                    } else {
                        // Jika file path adalah file, hapus file
                        unlink($filePath);
                    }
                }
            }
    ...
    public function proses_validasi($type)
    {
        ...
        $this->db->from("tbl_dispenda");
        $this->db->where("jabatan", $jabatan);
        $this->db->where("is_delete", "0");
        $data['ambil_ppat'] = $this->db->get()->row();

        $this->db->select("b.email, a.no_dokumen");
        $this->db->from("tbl_sptpd a");
        $this->db->join("tbl_wp b", "b.id_wp = a.id_wp", "left");
        $this->db->where("id_sptpd", $id);
        $data['ambil_wp'] = $this->db->get()->row();
        ...
        if ($type == 'PP') {
            ...
            if ($tgl_valid_dispenda == '') {
                ...
                if ($kode_validasi == trim($this->input->post('kode_approval'))) {
                    ...
                    $data_sptpd             = $this->pembayaran->get_data_sptpd($id_sptpd);
                    $tgl_pengajuan_berkas   = $data_sptpd->tanggal;
                    $jml_setor              = $data_sptpd->jumlah_setor;
                    $tahun_pengajuan_berkas = date("y", strtotime($tgl_pengajuan_berkas));
                    $VirtualAccount         = "10323000" . $tahun_pengajuan_berkas . sprintf('%06d', $id_sptpd);

                    $this->pembayaran->update_sptpd($id_sptpd, $id_user, $type, $tgl_valid_dispenda, $jabatan, null, null, $VirtualAccount);
                    $this->pembayaran->update_kode_validasi($id_sptpd, $kode_validasi, $type);

                    //post data request va jatim
                    $postdata = json_encode(array(
                        "VirtualAccount"    => $VirtualAccount,
                        "Nama"              => str_replace(array('" ', ' "', '"'), '', $data_sptpd->nama),
                        "TotalTagihan"      => $jml_setor,
                        "TanggalExp"        => date("Ymd", strtotime(date("Y-m-d") . ' + 14 days')),
                        "Berita1"           => $data_sptpd->no_pelayanan,
                        "Berita2"           => "",
                        "Berita3"           => "",
                        "Berita4"           => "",
                        "Berita5"           => "",
                    ...
                    $this->request_va($postdata);

                    // Request QRIS
                    #1. generate hash code
                    $merchantPan        = '9360011400001563087';        //DARI BANK JATIM
                    $billNumber         = $data_sptpd->no_dokumen;      //NO_DOKUMEN_SSPD
                    $terminalUser       = 'ID2024324548633';            //DARI BANK JATIM
                    $merchantHashKey    = 'TBHGYYKOYW';                 //DARI BANK JATIM
                    $hashcodeKey = hash(
                        'sha256',
                        $merchantPan . $billNumber . $terminalUser . $merchantHashKey
                    );

                    // Mendapatkan timestamp untuk 14 hari dari sekarang
                    $timestamp = strtotime('+13 days');

                    // Mengonversi timestamp ke format tanggal yang diinginkan
                    $empatBelasHariKemudian = date('Y-m-d', $timestamp);

                    $postdataQris = json_encode(json_decode('{
                        "merchantPan": "9360011400001563087",
                        "hashcodeKey": "' . $hashcodeKey . '",
                        "billNumber": "' . $data_sptpd->no_dokumen . '",
                        "purposetrx": "BPHTB",
                        "storelabel": "Bapenda Kota Malang",
                        "customerlabel": "' . substr(str_replace(array('" ', ' "', '"'), '', $data_sptpd->nama), 0, 20) . '",
                        "amount": ' . $jml_setor . ',
                        "terminalUser": "ID2024324548633",
                        "expiredDate": "' . $empatBelasHariKemudian . ' 23:59:00"
                    }'));

                    $data_log_qris = array(
                        "tanggal_qris"  => date("Y-m-d H:i:s"),
                        "content"       => $postdataQris,
                        "req_resp"      => 0,
                    );


                    $response = $this->request_qris($postdataQris);
                    $this->pembayaran->insert_log_qris($data_log_qris);
                    if ($response) {
                        $responseData   = json_decode($response);
                        if ($responseData->responsCode == '00') {
                            $qrValue        = $responseData->qrValue;
                            $invoice_number = $responseData->invoice_number;
                            $billNumber = $responseData->billNumber;

                            $data_update_qr = array(
                                "qr_code"           => $qrValue,
                                "qr_invoice_number" => $invoice_number,
                                "qr_bill_number"    => $billNumber,
                            );
                            $this->pembayaran->update_sptpd_qris($id_sptpd, $data_update_qr);
                            file_put_contents(FCPATH . "/assets/log/sukses_qris.log", date("Y-m-d H:i:s") . "- curl_response : " . $response . "- post_data : " . $postdataQris . PHP_EOL, FILE_APPEND);
                        } else {
                            file_put_contents(FCPATH . "/assets/log/gagal_qris.log", date("Y-m-d H:i:s") . "- curl_response : " . $response . "- post_data : " . $postdataQris . PHP_EOL, FILE_APPEND);
                        }
                    } else {
                        file_put_contents(FCPATH . "/assets/log/gagal_qris.log", date("Y-m-d H:i:s") . "- curl_response : " . $response . "- post_data : " . $postdataQris . PHP_EOL, FILE_APPEND);
                    }
    ...
    public function request_va($postdata)
    {
        $result = true;
        $url_va_jatim = "https://jatimva.bankjatim.co.id/Va/RegPen";
        $ch = curl_init($url_va_jatim);
    ...
    public function request_qris($postdata)
    {
        $result = true;
        $url_qris_jatim = "https://jatimva.bankjatim.co.id/MC/Qris/Dynamic";
        $ch = curl_init($url_qris_jatim);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $postdata);
    ...
    public function simpan_verifikasi_bphtb($id_sptpd)
    {
        $response = array(
            "status"    => false,
            "msg"       => ""
        ...
        if ($id_sptpd != "") {
            $sptpd  = $this->mod_sptpd->get_sptpd($id_sptpd);
            $data   = $this->input->post();
            if ($data) {
                #1 cek hasil yang dipilih
                $hasil = ($data["verifikasi_hasil"] == "hasil1") ? "Berkas Lengkap" : $data["verifikasi_hasil_custom"];

                #2 jika jika $hasil berisi kosong
                if ($hasil != "") {
                    if ($data["verifikasi_tanggal_verifikasi"] != "") {
                        #3 simpan / update tabel sptpd
                        $data_verifikasi = array(
                            "tgl_verifikasi"    => date("Y-m-d", strtotime($data["verifikasi_tanggal_verifikasi"])),
                            "id_verifikator"    => $this->session->userdata('s_id_user'),
                            "hasil_verifikasi"  => $hasil,
                        );
                        $simpan_verifikasi = $this->mod_sptpd->simpan_verifikasi_bphtb($data_verifikasi, $id_sptpd);
                        if ($simpan_verifikasi["status"]) {
                            $no_dokumen = str_replace(".", "", $sptpd->no_dokumen);
                            if ($no_dokumen != "") {
                                if ($_FILES['verifikasi_lhp_verlap']['name'] != '') {
                                    $file = $this->uploadFile('verifikasi_lhp_verlap', 'lhp_verlap_' . $id_sptpd, $no_dokumen, 'jpg|pdf');
                                    $paramVerifikasi['verifikasi_lhp_verlap'] = $file;
                                }

                                if ($_FILES['verifikasi_lampiran_lain']['name'] != '') {
                                    $file = $this->uploadFile('verifikasi_lampiran_lain', 'lampiran_lain_' . $id_sptpd, $no_dokumen, 'jpg|pdf');
                                    $paramVerifikasi['verifikasi_lampiran_lain'] = $file;
                                }
                            }

                            $response = array(
                                "status"    => true,
                                "msg"       => "Data verifikasi bphtb berhasil disimpan."
                            );
                        } else {
                            $response = array(
                                "msg" => "Data verifikasi bphtb gagal disimpan"
                            );
                        }
                    } else {
                        $response = array(
                            "msg" => "Field tanggal verifikasi tidak boleh kosong.!"
                        );
                    }
                } else {
                    $response = array(
                        "msg" => "Field hasil tidak boleh kosong.!"
                    );
                }
            } else {
                $response = array(
