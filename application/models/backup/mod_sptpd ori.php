...
class Mod_sptpd extends CI_Model
{
    ...
    public function get_sptpd1($id = '', $limit = '', $go_page = '', $start = '', $halt_at = '', $nik = '', $nop = '', $nodok = '', $no_pelayanan = '', $date_start = '', $date_end = '')
    {
        if ($id != '') {
            $this->db->where('id_sptpd', $id);
            $this->db->order_by('id_sptpd', 'desc');
...
return $query->result();
    ...
    public function get_notif()
    {
        ...
        if ($this->session->userdata('s_tipe_bphtb') == 'D') {
            $where = array('flag_dispenda' => 1);
            $data = $this->db->get_where($this->tbl, $where)->num_rows();

            return $data;
        } else if ($this->session->userdata('s_tipe_bphtb') == 'PT') {
            $where = array('flag_ppat' => 1);
            $data = $this->db->get_where($this->tbl, $where)->num_rows();
    ...
    public function get_notif2()
    {
        ...
        //ori $data = $this->db->query("select * from tbl_user where is_blokir = '1'")->num_rows();
        $data = $this->db->query("SELECT * from tbl_wp join tbl_user ON tbl_user.id_user=tbl_wp.id_user where tbl_user.is_blokir = '1' and tbl_user.tipe='WP' and tbl_wp.status='1'")->num_rows();
        return $data;
    }
