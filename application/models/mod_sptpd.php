...
class Mod_sptpd extends CI_Model
{
    ...
    public function get_sptpd($id = '', $limit = '', $go_page = '', $start = '', $halt_at = '', $nik = '', $nop = array(), $id_ppat = '', $nodok = '', $no_pelayanan = '', $date_start = '', $date_end = '')
    {
        if ($id != '') {
    ...
    public function get_nik($id = '', $limit = '', $go_page = '', $start = '', $halt_at = '', $nik = '')
    {
        if ($id != '') {
            ...
            $this->db->select("a.*, b.id_wp");
            $this->db->from("tbl_nik a");
            $this->db->join("tbl_wp b", "b.nik = a.nik", "left");
    ...
    */
    ...
    public function add_sptpd_dispenda($id_ppat, $nik, $wajibpajak, $nop = '', $nop_alamat, $nilai_pasar, $jenis_perolehan, $ptsl, $npop, $npoptkp, $dasar_jml_setoran, $nomor_jml_setoran, $tgl_jml_setoran, $hitung_sendiri_jml_setoran, $custom_jml_setoran, $jml_setor, $tanggal, $no_dokumen, $nop_pbb_baru, $kode_validasi, $id_user, $id_pp, $picture = '', $luas_tanah_op, $luas_bangunan_op, $luas_tanah_b_op, $luas_bangunan_b_op, $njop_tanah_op, $njop_bangunan_op, $njop_tanah_b_op, $njop_bangunan_b_op, $njop_pbb_sptpd, $text_no_sertifikat, $text_lokasi_op, $text_thn_pajak_sppt, $tanah_inp_aphb1, $tanah_inp_aphb2, $tanah_inp_aphb3, $bangunan_inp_aphb1, $bangunan_inp_aphb2, $bangunan_inp_aphb3, $tanah_b_inp_aphb1, $tanah_b_inp_aphb2, $tanah_b_inp_aphb3, $bangunan_b_inp_aphb1, $bangunan_b_inp_aphb2, $bangunan_b_inp_aphb3, $nama_nik, $alamat_nik, $text_propinsi, $text_kotakab, $text_kecamatan, $text_kelurahan, $rtrw_nik, $kodepos_nik, $no_pelayanan, $nama_petugas_lapangan = '', $kurang_bayar = '', $sspd_lama = '')
    {
        $kd_propinsi  = $nop[0];
        $kd_kabupaten = $nop[1];
        $kd_kecamatan = $nop[2];
    ...
    public function get_lampiran($no_dokumen)
    {
        $this->db->select('
            ...
            tbl_formulir_penelitian.lampiran_spoplspop
            ');
        $this->db->where('no_dokumen', $no_dokumen);
    ...
    public function get_notif2()
    {
        ...
        $this->db->join("tbl_wp", "tbl_wp.id_user = tbl_user.id_user");
        $this->db->where("tbl_user.is_blokir = '1' and tbl_wp.status = '1'");
    ...
    public function get_notif_jml_tte()
    {
        ...
        if ($type_user == 'D' && $jabatan == 2) {
            ...
            $this->db->where("tanggal > '" . date('Y-m-01', strtotime('-12 months')) . "'");
            $this->db->order_by('id_sptpd', 'desc');
            $query = $this->db->get($this->tbl);
            return $query->num_rows();
        }
    ...
    public function update_file_validasi($id_sptpd, $data_to_update)
    {
        $this->db->where("id_sptpd", $id_sptpd);
        $update_file_validasi = $this->db->update("tbl_sptpd", $data_to_update);
        return $update_file_validasi;
    }

    public function get_nik_user_kabid($id_user)
    {
        $this->db->select("tu.*, td.jabatan");
        $this->db->from("tbl_user tu");
        $this->db->join("tbl_dispenda td", "td.id_user=tu.id_user", "left");
        $this->db->where("tu.id_user", $id_user);
        $this->db->where("td.jabatan", "2");
        return $this->db->get()->row();
    }

    public function get_file_validasi_name($id_sptpd)
    {
        $this->db->select("ts.file_validasi");
        $this->db->from("tbl_sptpd ts");
        $this->db->where("ts.id_sptpd", $id_sptpd);
        return $this->db->get()->row();
    }

    public function simpan_verifikasi_bphtb($data_verifikasi, $id_sptpd)
    {
        // Mulai transaksi
        $this->db->trans_begin();
        ...
        $simpan = $this->db->update("tbl_sptpd", $data_verifikasi);
